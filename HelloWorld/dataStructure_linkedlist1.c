...
typedef struct ListNode { 
	int data; // 데이터필드 부분 정수형 변수 사용해 공간 할당
	struct ListNode* link; // 다음 노드 주소값 저장, ListNode포인터 사용, 노드 전체를 가리키는 포인트 변수
}listNode;
...
typedef struct { 
	listNode* head; // 자료형 ListNode를 가리킬 수 있도록 정의
}linkedList_h;


// 연결리스트 생성====================================================================
linkedList_h* createLinkedList_h(void) { 
	linkedList_h* H; // 헤드노드 생성
...
void addNode(linkedList_h* H, int x) { 
	...
	// 구조체 listNode사용해 새노드, 마지막 노드 선언
	listNode* NewNode; // 새 노드 
	listNode* LastNode; // 연결리스트 노드 많은 경우 마지막 노드 찾기 위한 변수
	...
	// 구조체listNode를 이용해 생성한 NewNode의 멤버변수 data, link에 값 할당
...
void deleteNode(linkedList_h* H) { 
	listNode* prevNode; // 삭제되는 노드의 선행노드 선언, 자료형 ListNode를 가리킴
	listNode* delNode; // 삭제되는 노드 선언
